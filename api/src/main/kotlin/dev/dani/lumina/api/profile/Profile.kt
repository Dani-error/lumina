...
interface Profile {
    ...
    companion object {
        ...
        fun unresolved(name: String): Unresolved =
            DefaultUnresolvedProfile(name, null)

        fun unresolved(uniqueId: UUID): Unresolved =
            DefaultUnresolvedProfile(null, uniqueId)
        ...
        fun skin(name: String, value: String, signature: String): Resolved = resolved(name, UUID.randomUUID(), setOf(
            ProfileProperty.property("textures", value, signature)
        ))
...
internal data class DefaultResolvedProfile(
    ...
    override val properties: Set<ProfileProperty> =
        if (rawProperties.isEmpty()) emptySet() else rawProperties.toSet()
...
internal data class DefaultUnresolvedProfile(
    ...
    override fun hashCode(): Int =
        Objects.hash(name, uniqueId)

    override fun equals(other: Any?): Boolean = safeEquals<Profile.Unresolved>(this, other) { orig, comp ->
        Objects.equals(orig.name, comp.name) &&
                Objects.equals(orig.uniqueId, comp.uniqueId)
    }
}